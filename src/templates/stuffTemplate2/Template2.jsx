...
const Template2 = ({ data }) => {
  return (
    <div className="invoice template2">
      {/* Header */}
          ...
          {data.items && Array.isArray(data.items) && data.items.length > 0 ? (
            data.items.map((item, i) => (
              <tr key={i}>
                ...
                <td>{item.qty || 0}</td>
                <td>{data.currentSymbol}{parseFloat(item.amount || 0).toFixed(2)}</td>
                <td>{data.currentSymbol}{parseFloat(item.total || 0).toFixed(2)}</td>
              </tr>
            ))
          ) : (
      ...
      <div className="totals">
        ...
        <h3><b>Total:</b> {data.currentSymbol}{data.total.toFixed(2)}</h3>
      </div>
